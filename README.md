...
§5.5). 새 세션이나 서브에이전트에 보고서만 주고 표적 반박을 시키는 것을
권장한다. (장래 `/penguin-verify` 스킬 후보.)
...
- 실전 발동 1건 (2026-08-14, 턴 종료 감지 재검토): 미검토 공식 인터페이스
  발견과 "실측 먼저" 안전판은 실전 재현, 처방 정밀도는 미달(절반 과장) —
  그 오차에서 나온 템플릿 보강 6건이 현 SKILL.md에 반영됨. 상세:
  `benchmarks/results/2026-08-14-turn-end-live-case.md`
